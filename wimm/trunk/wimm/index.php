...
<script language="JavaScript" type="text/JavaScript">
    function sel_row(row_id)
    {
        ...
        var s1;
        var field = document.getElementById("HIDDEN_ID");
        field.value = row_id;
        if(row_id.length<1) {
            field = document.getElementById("OK_BTN");
            field.value = "Добавить";
            field = document.getElementById("FRM_MODE");
            field.value = "insert";
        }
        else    {
            field = document.getElementById("OK_BTN");
            field.value = "Изменить";
            field = document.getElementById("FRM_MODE");
            field.value = "update";
...
/*		for(i=0; i<coll.length; i++)             {
			...
			if(s_1=="ID")	{
				if(v.checked)	{
					...
					expenses.FRM_MODE.value="update";
					break;
				}
			}
		}*/
...
if($conn)	{
	...
	if(strcmp($fm,"insert")==0)	{
		...
		$s = getRequestParam("t_name","Покупка!");
		$sql .= "'$s',";
		...
		$s = getRequestParam("HIDDEN_ID",0);
		$sql = "delete from m_transactions where transaction_id=$s";
	}
	$cd = getdate();
	$m = $cd['mon'];
	$y = $cd['year'];
