<?php
	...
	if(strlen($user_name)>0)	{
	    ...
	    if ($conn) {
                $a_params = [];
                $a_params['uname'] = value4db($user_name);
                $a_params['upasswd'] = value4db(getRequestParam("UPASS", ""));
                $sql = formatSQL($conn, 
                        "select user_id as cnt, user_name from m_users "
                        . "where user_login=:uname and "
                        . "user_password=#PASSWORD#(:upasswd)");
                ...
                if ($stmt) {
                    if ($stmt->execute($a_params)) {
                        ...
                        if ($row) {
                            ...
                            if (strpos($posted_date, '05') === 0) { // day first
...
<html>
    <head>
        ...
        <link rel="STYLESHEET" href="css/jquery_autocomplete_ifd.css" 
              type="text/css"/>
...
?>
        ...
        <form name="auth" action="index.php" method="post">
            <P>Вы должны автоматически перейти на другую страницу. 
                Если этого не происходит нажмите на кнопку</P>
...
?>
        <script language="JavaScript" type="text/JavaScript">
            function bodyOnLoad()
            {
                document.getElementById('dtst').value = 
                        (new Date(2015,01,05)).toLocaleString();
            }
        </script>
<?php
